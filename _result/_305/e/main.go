...
func main() {
	...
	for i := 0; i < k; i++ {
		...
		pq[i] = &PriorityQueueItem{node: p, hp: h, index: i}
		d[p] = h
	}
	...
	for pq.Len() > 0 {
		...
		if item.hp < d[item.node] {
			continue
		}
		for _, e := range g.nodes[item.node] {
			if d[e.to] < item.hp-1 && item.hp-1 >= 0 {
				d[e.to] = item.hp - 1
				heap.Push(&pq, &PriorityQueueItem{node: e.to, hp: item.hp - 1})
			}
		}
	}
