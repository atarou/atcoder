...
func main() {
	...
	for {
		...
		for i := 0; i < n; i++ {
			x, y, z := nf3()
			r := nf()
			xyzrs[i] = xyzr{x, y, z, r}
		}

		dists := make([]ftd, 0)
		for i := 0; i < n; i++ {
			for j := i + 1; j < n; j++ {
				d := math.Sqrt(math.Pow((xyzrs[i].x)-(xyzrs[j].x), 2)+math.Pow((xyzrs[i].y)-(xyzrs[j].y), 2)+math.Pow((xyzrs[i].z)-(xyzrs[j].z), 2)) - xyzrs[i].r - xyzrs[j].r
				v := ftd{i, j, MaxIn2Values(0, d)}
				dists = append(dists, v)
			}
		}
		g := NewKruskalGraph(n)
		for i := 0; i < len(dists); i++ {
			g.AppendEdge(dists[i].from, dists[i].to, dists[i].d)
		}
		ans := g.kruskal()
		numRounded := roundToDecimalPlaces(ans, 3)
		fmt.Printf("%.3f\n", numRounded)
	}
}
func roundToDecimalPlaces(n float64, decimalPlaces int) float64 {
...
func nf3() (float64, float64, float64) {
	return nf(), nf(), nf()
}
...
func MaxIn2Values(a float64, b float64) float64 {
	if a < b {
		return b
	}
	return a
...
// AddEdge adds a edge connects vertex u to v and v to u.
func (g *kruskalGraph) AppendEdge(from, to int, cost float64) {
...
func (g *kruskalGraph) kruskal() float64 {
