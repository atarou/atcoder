...
func main() {
	...
	for l := 0; l < n; l++ {
		for r := l; r < n; r++ {
			...
			newa := make([]int, 0, n)
			if l > 0 {
				newa = append(newa, aa[:l]...)
			}
			temp := make([]int, r-l+1)
			copy(temp, aa[l:r+1])
			reversei(temp)
			// out("temp[l:r]:", temp[l:r+1])
			newa = append(newa, temp...)
			// out("newa:", newa)
			if r < n-1 {
				newa = append(newa, aa[r+1:]...)
			}
			// out("aa:", newa)
			ss = append(ss, newa)
		}
	...
	// out(ss)
	sst := make([]string, 0)
	for _, st := range ss[k-1] {
		sst = append(sst, itoa(st))
	}
	out(strings.Join(sst, " "))
}
