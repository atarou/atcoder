...
func main() {
	...
	for _, ab := range temp {
		...
		gg.AppendEdge(a, b, bcost)
		gg.AppendEdge(b, a, acost)
	}
	ans := gg.dijkstra(0, n-1)
	out(ans - int64(p))
}
...
func (graph *graph) dijkstra1(start, s int) []int {
	//各ノードにかかるコスト
...
func (graph *graph) dijkstra(start, goal int) int64 {
	//各ノードにかかるコスト
	...
	for pq.Len() > 0 {
		...
		if dist[v] < int64(item.distance) {
			continue
		}
		for _, edge := range graph.nodes[v] {
			nextDist := dist[v] + int64(edge.cost)
			if nextDist < dist[edge.to] {
				dist[edge.to] = nextDist
				heap.Push(&pq, &PriorityQueueItem{node: edge.to, distance: int(nextDist)})
			}
		}
	}
