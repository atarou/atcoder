...
func main() {
	...
	for i := 0; i < (1 << r); i++ {
		aaacopy := make([][]bool, r)
		for j := 0; j < r; j++ {
			aaacopy[j] = make([]bool, c)
			for index, v := range aaa[j] {
				aaacopy[j][index] = v
			}
		}
		// out("aaacopy before:", aaacopy)
		for j := 0; j < r; j++ {
			if (i>>j)&1 == 1 {
				// out("j番目がひっくり返っている:", j)
				for k := 0; k < c; k++ {
					aaacopy[j][k] = !aaacopy[j][k]
				}
			}
		}
		// out("aaacopy after:", aaacopy)
		ok := 0
		for j := 0; j < c; j++ {
			tcnt := 0
			fcnt := 0
			...
			ok += MaxIn2Values(tcnt, fcnt)
		}
		// if ok == 8 {
		// 	out(aaacopy)
		// }
		ans = MaxIn2Values(ans, ok)
	}
	out(ans)
}
