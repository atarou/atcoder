...
func main() {
	...
	sort.Slice(ansstars, func(i, j int) bool {
		if ansstars[i].x == ansstars[j].x {
			return ansstars[i].y < ansstars[j].y
		} else {
			return ansstars[i].x < ansstars[j].x
		}
	})
	diffs := make([]xy, m-1)
	for i := 0; i < m-1; i++ {
		diffx := ansstars[i+1].x - ansstars[i].x
		diffy := ansstars[i+1].y - ansstars[i].y
		diffs[i] = xy{diffx, diffy}
	}
	// out("diffs:", diffs)
	n := ni()
	stars := make([]xy, n)
	...
	for i := 0; i < n; i++ {
		...
		for j := 0; j < m-1; j++ {
			for k := from + 1; k < n; k++ {
				...
				// 	out("targety:", targety)
				// }
				if targetx == basex+diffs[j].x && targety == basey+diffs[j].y {
					basex = targetx
					basey = targety
					from = k
					cnt++
	...
	// out("record:", record)
	xd := stars[record].x - ansstars[0].x
	yd := stars[record].y - ansstars[0].y
	out(xd, yd)
}
