...
func main() {
	...
	var rec func(s, ve uint32) int
	rec = func(s, ve uint32) int {
		if dp[s][ve] != -1 {
			return dp[s][ve]
		}
		if s == uint32(max) && ve == 0 {
			dp[s][ve] = 0
			return 0
		}
		res := math.MaxInt32
		for u := 0; u < v; u++ {
			visited := (s>>uint32(u))&1 == 1
			if !visited {
				r := rec(s^(1<<uint32(u)), uint32(u)) + g[ve][u]
				res = MinIn2Values(res, r)
			}
		}
		dp[s][ve] = res
		return res
