...
func main() {
	...
	for i := 0; i < n; i++ {
		h, s := ni2()
		max = MaxIn2Values(max, h+s*(n-1))
		hshs[i] = hs{int64(h), int64(s)}
	}
	ans := bs(int64(0), int64(max), hshs)
	out(ans)
}

func bs(start, end int64, hshs []hs) int64 {
	u := end
	l := start
	for u > l {
		x := (u + l) / 2
...
func judge(x int64, hshs []hs) bool {
	tt := make([]int64, len(hshs))
	for i := 0; i < len(hshs); i++ {
		hs := hshs[i]
		if x-hs.h < 0 {
			return false
		}
