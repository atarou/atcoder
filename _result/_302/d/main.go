...
func main() {
	...
	for i := int64(0); int64(i) < n; i++ {
		...
		if int64(in) == m {
			in--
			val1 := int64(-1)
			dif1 := int64(aa[i]) - int64(bb[in])
			if dif1 <= int64(d) {
				val1 = int64(aa[i]) + int64(bb[in])
			}
			ans = MaxIn2Values(ans, int64(val1))
		} else {
			val1 := int64(-1)
			dif1 := bb[in] - aa[i]
			if dif1 <= d {
			...
			dif2 := int64(0)
			if in-1 >= 0 {
				dif2 = int64(aa[i]) - int64(bb[in-1])
				if dif2 <= int64(d) {
					val2 = int64(aa[i]) + int64(bb[in-1])
				}
			}
			val := MaxIn2Values(int64(val1), int64(val2))
			ans = MaxIn2Values(int64(ans), val)
		}
	}
	out(ans)
}
...
func ni2() (int64, int64) {
	return ni(), ni()
}
func ni3() (int64, int64, int64) {
	return ni(), ni(), ni()
}

func nis(arg ...int64) []int64 {
	n := arg[0]
	t := int64(0)
	if len(arg) == 2 {
		t = arg[1]
...
func MaxIn2Values(a int64, b int64) int64 {
	if a < b {
		return int64(b)
	}
	return int64(a)
