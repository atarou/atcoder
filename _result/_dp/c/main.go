...
func main() {
	...
	dp := make([][3]int, n)
	dp[0] = [3]int{abc[0][0], abc[0][1], abc[0][2]}
	for i := 1; i < n; i++ {
		a := int(math.Max(float64(dp[i-1][1]), float64(dp[i-1][2])))
		b := int(math.Max(float64(dp[i-1][0]), float64(dp[i-1][2])))
		c := int(math.Max(float64(dp[i-1][0]), float64(dp[i-1][1])))
		dp[i] = [3]int{a + abc[i][0], b + abc[i][1], c + abc[i][2]}
	}
	out(max(dp[n-1]))
}
