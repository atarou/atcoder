...
func main() {
	...
	for i := 0; i < m; i++ {
		...
		x--
		y--
		aaa[x] = append(aaa[x], y)
		aaa[y] = append(aaa[y], x)
	}
	ans := 0
	for i := 0; i < (1 << n); i++ {
		selected := make([]int, 0)
		...
		for i, k := range selected {
			for ii, kk := range selected {
				if i == ii {
					continue
				}
				//もしkkがaaa[k]に含まれていなかったらアウト
				ok := contain(aaa[k], kk)
				if !ok {
					ng = true
					break
		...
		if !ng {
			ans = MaxIn2Values(ans, len(selected))
		}
	}
	out(ans)
}
func contain(ii []int, i int) bool {
	for _, v := range ii {
		if i == v {
			return true
		}
