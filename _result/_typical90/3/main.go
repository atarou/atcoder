...
func main() {
	...
	for i := 0; i < n-1; i++ {
		a, b := ni2()
		g.AppendEdge(a-1, b-1)
		g.AppendEdge(b-1, a-1)
	}
	var dfs func(now, p, cnt, base int, seen []bool)
	long := 0
	dfs = func(now, p, cnt, base int, seen []bool) {
		seen[now] = true
		connectbase := false
		for _, e := range g.e[now] {
			...
			// out("e.to", e.to)
			// out(seen)
			dfs(e.to, now, cnt+1, base, seen)
		}
		//  探索するものがなくなった
		// out("探索するものがない", "now", now)
		// out("base: ", base)
		if !connectbase {
			// out("さらに", base, "とも繋がっていない", now)
			if long < cnt {
				long = cnt
			}
		}
