...
func main() {
	...
	for _, e := range ee {
		T := e % t
		sita := 2.0 * float64(T) * math.Pi / float64(t)
		Y := -float64(l) * math.Sin(sita) / 2
		Z := float64(l) * (1 - math.Cos(sita)) / 2
		tan := Z / math.Sqrt(float64(x)*float64(x)+(Y-float64(y))*(Y-float64(y)))
		thetaRad := math.Atan(tan)
		out(thetaRad * (180.0 / math.Pi))
	}
}
