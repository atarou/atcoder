...
func main() {
	...
	for i := 0; i < n-1; i++ {
		for j := i + 1; j < n; j++ {
			dij := (xyxy[i].x-xyxy[j].x)*(xyxy[i].x-xyxy[j].x) + (xyxy[i].y-xyxy[j].y)*(xyxy[i].y-xyxy[j].y)
			g.AppendEdge(i, j, dij)
			g.AppendEdge(j, i, dij)
		}
	}

	virused := make([]bool, n)
	virused[0] = true
	var dfs func(now, p int)
	dfs = func(now, p int) {
		for _, e := range g.nodes[now] {
			if e.to == p {
				continue
			}
			if virused[e.to] {
